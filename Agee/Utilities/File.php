...
class File
{
    ...
    public static function saveThumbnail($file, $name, $size = 250, $path = '/uploads/')
    {
        $uploadFile = $path . '' . $name . '.jpg';
        //TODO: Obsługa rozszerzeń itp.
        //TODO: zmiana rozmiaru obrazka
    ...
    public static function saveUploadedSong($file, $path = 'upload/',$folder = '')
    {
        if($folder != '')
            if(!is_dir($path.$folder))
                mkdir($path.$folder);
        ...
        $exp = explode('.', $file['name']);
        $actualName = str_replace('.mp3','',$file['name']);
        $extension = $exp[count($exp) - 1];
        $uploadFile = $path . $folder . '' . md5($file['name'] . '-' . mktime()) . '.' . $extension;

        if (move_uploaded_file($file['tmp_name'], $uploadFile))
            return ['/'.$uploadFile, $actualName];
        else
            return false;
    }

    public static function saveRawImage($data, $path = 'upload/',$folder = '')
    {

        list($type, $data) = explode(';', $data);
        list(, $data)      = explode(',', $data);
        $data = base64_decode($data);
        $tmpFilename = rand(0,10000).'.png';
